...
function frontDoorPassword(word) {
    const FIRST_WORD = word[0];
    const CAPITALIZE_WORD = FIRST_WORD.toUpperCase() + word.slice(1).toLowerCase()
...
// Implement the function backDoorResponse that takes a line of the poem as an argument and 
// returns the last letter of that line that is not a whitespace character.

function backDoorResponse(line) {
    const TRIM_LINE = line.trim()
    return TRIM_LINE[TRIM_LINE.length - 1]
}
...
// Implement the function backDoorPassword that accepts a string (the combined letters you found in task 3) and 
// returns the polite version of the capitalized password.

function backDoorPassword(word) {
    const FIRST_WORD = word[0];
    const CAPITALIZE_WORD = FIRST_WORD.toUpperCase() + word.slice(1) + ", please"
    return CAPITALIZE_WORD
}
