...
// When you have lasagna in the oven, you want to know whether you can already take it out or not. To make sure the lasagna does not burn in the oven, 
// you usually set a timer. But sometimes you forget about that.
...
function preparationTime(layers, time) {
  return !time ? layers.length * 2 : layers.length * time;
}
...
// Task 3: Compute the amount of noodles and sauce needed
...
// Define the function quantities that takes an array of layers as a parameter. The function will then determine the quantity of noodles and 
// sauce needed to make your meal. The result should be returned as an object with keys noodles and sauce.

function quantities(manyLayers) {
  let noodles = 0;
  let sauce = 0;
  for (let i = 0; i < manyLayers.length; i++) {
    if (manyLayers[i] === 'noodles') {
      noodles++;
    } else if (manyLayers[i] === 'sauce') {
      sauce++;
    }
  }
  const items = {
